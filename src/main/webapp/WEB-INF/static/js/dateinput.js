...
// require("dateinput/date_input.css");
    ...
    DateInput.prototype = {
        build : function() {
            var monthNav = $('<p class="month_nav">'
                + '<span class="button prev" title="按 [PageUp] 上月">&#171;</span>'
                + ' <span class="month_name"></span> '
                + '<span class="button next" title="按 [PageDown] 下月">&#187;</span>'
                + '</p>');
            ...
            var yearNav = $('<p class="year_nav">'
                + '<span class="button prev" title="按 [Ctrl+PageUp] 上一年">&#171;</span>'
                + ' <span class="year_name"></span> '
                + '<span class="button next" title="按 [Ctrl+PageDown] 下一年">&#187;</span>'
                + '</p>');
        ...
        selectMonth : function(date) {
            ...
            if (!this.currentMonth
                || !(this.currentMonth.getFullYear() == newMonth
                .getFullYear() && this.currentMonth.getMonth() == newMonth
                .getMonth())) {
                this.currentMonth = newMonth;
                var rangeStart = this.rangeStart(date), rangeEnd = this
                    .rangeEnd(date);
                var numDays = this.daysBetween(rangeStart, rangeEnd);
                var dayCells = "";
                for (var i = 0; i <= numDays; i++) {
                    var currentDay = new Date(rangeStart.getFullYear(),
                        rangeStart.getMonth(), rangeStart.getDate() + i,
                        12, 00);
                    if (this.isFirstDayOfWeek(currentDay))
                        dayCells += "<tr>";
                    if (currentDay.getMonth() == date.getMonth()) {
                        if($(this.input).hasClass("today")){
                            if(currentDay<= new Date()){
                                dayCells += '<td class="selectable_day" date="'
                                + this.dateToString(currentDay) + '">'
                ...
                this.yearNameSpan.empty().append(this.currentMonth
                    .getFullYear());
                $(".selectable_day", this.tbody).click(this.bindToObj(function(
                    event) {
                    this.changeInput($(event.target).attr("date"));
                }));
                $("td[date=" + this.dateToString(new Date()) + "]", this.tbody)
                    .addClass("today");
            ...
            $('td[date=' + this.selectedDateString + ']', this.tbody)
                .addClass("selected");
        },
        ...
        changeInput : function(dateString) {
            this.input.val(dateString).change().blur();
            this.hide();
        },
        show : function() {
        ...
        stringToDate : function(string) {
            ...
            if (matches = string.match(/^(\d{1,2}) ([^\s]+) (\d{4,4})$/)) {
                return new Date(matches[3], this.shortMonthNum(matches[2]),
                    matches[1], 12, 00);
            } else {
                return null;
            };
        ...
        dateToString : function(date) {
            return date.getFullYear() + '-'
                + this.short_month_names[date.getMonth()] + '-'
                + date.getDate();
        },
        setPosition : function() {
            ...
            this.rootLayers.css({
                top : offset.top + this.input.outerHeight()-parentOffset.top,
        ...
        moveDateMonthBy : function(amount) {
            var newDate = new Date(this.selectedDate.getFullYear(),
                this.selectedDate.getMonth() + amount, this.selectedDate
                    .getDate());
        ...
        moveMonthBy : function(amount) {
            var newMonth = new Date(this.currentMonth.getFullYear(),
                this.currentMonth.getMonth() + amount, this.currentMonth
                    .getDate());
        ...
        daysBetween : function(start, end) {
            start = Date.UTC(start.getFullYear(), start.getMonth(), start
                .getDate());
    ...
    $.fn.dateinput = function(opts) {
        return this.each(function() {
            if( $(this).data("dateinput-type") != true ){
    ...
    $("input.date-input").live("mouseenter",function(){
        $(this).dateinput();
    });
    $("input.date-input").live("keypress",
        function() {
            return false;
        });
